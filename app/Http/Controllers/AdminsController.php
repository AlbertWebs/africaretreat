...
public function tags(){
    ...
    $page_title = 'list';
    $page_name = 'Tag';
    return view('admin.tag',compact('page_title','Tag','page_name'));
}
...
public function edit_Tag(Request $request, $id){
    ...
    DB::table('types')->where('id',$id)->update($updateDetails);
    Session::flash('message', "Changes have been saved");
    return Redirect::back();
}

public function addSlung(){
    $Types = DB::table('types')->get();
    foreach($Types as $type){
        $updateDetails = array(
            'slung'=> Str::slug($type->title)
        );
        DB::table('types')->where('id',$type->id)->update($updateDetails);
    }
}
